...
export function BottomNav({ tripId, active }: { tripId: string; active: string }) {
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-30 border-t border-border bg-card/95 pb-safe backdrop-blur-sm"
      aria-label="メインナビゲーション"
    >
      <div className="mx-auto flex max-w-md items-center justify-around py-1">
        {navItems.map((item) => {
          const href =
            item.key === 'home' ? '/' : buildTripPageHref(tripId, item.key as TripPageSection)
